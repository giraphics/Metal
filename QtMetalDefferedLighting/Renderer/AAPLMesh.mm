                                              ...
                                              metalDevice:(nonnull id<MTLDevice>)device
{
    ...
    // HACK! to allow the textures to suffice
    ///////////////////////////////////
    QImage albedo("://Images/Temple/StructureBaseColor.png");
    MTLTextureDescriptor* textureDescriptor = [[MTLTextureDescriptor alloc] init];
    textureDescriptor.textureType = MTLTextureType2D;
    textureDescriptor.pixelFormat = MTLPixelFormatRGBA8Unorm;
    textureDescriptor.width = albedo.size().width();
    textureDescriptor.height = albedo.size().height();

    const uint8_t* imageBits = albedo.bits();
    ...
    stbi_uc* pPixels = NULL;
    {
        ...
        if (tempDir.isValid())
        {
          ...
          if (QFile::copy("://Images/Temple/StructureBaseColor.png", tempFile))
          {
    ...
    NSUInteger bytesPerRow = 4 * albedo.width();

    MTLRegion region = { { 0, 0, 0 }, {(NSUInteger)albedo.width(), (NSUInteger)albedo.height(), 1} };
...
//           property.type == MDLMaterialPropertyTypeURL)
        {
            ...
            NSLog(@"##############RSC Address: %@", property.stringValue);
            NSLog(@"##############ABS Address: %@", [url absoluteString]);
