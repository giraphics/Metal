...
static const NSUInteger AAPLNumFairyVertices = 4;
...
extern id <MTLLibrary> getLibrary(const QString& p_ShaderFile, id <MTLDevice> device);

#define LOAD_FAIRY_RESOURCE_WITH_QIMAGE 0 // Flag to demonstrate use of Qt resource file.
...
- (void)loadMetal
{
    ...
    {
        {
            id <MTLLibrary> gBufferLibrary = getLibrary(QString("://Shaders/libs/AAPLGBuffer.metallib"), _device);
            assert(gBufferLibrary);

            id <MTLFunction> GBufferVertexFunction = [gBufferLibrary newFunctionWithName:@"gbuffer_vertex"];
            id <MTLFunction> GBufferFragmentFunction = [gBufferLibrary newFunctionWithName:@"gbuffer_fragment"];
    ...
    // Setup render state to apply directional light and shadow in final pass
    {
        ...
        {
            id <MTLLibrary> deferredLightLibrary = getLibrary(QString("://Shaders/libs/AAPLDirectionalLight.metallib"), _device);
            assert(deferredLightLibrary);

            id <MTLFunction> directionalVertexFunction = [deferredLightLibrary newFunctionWithName:@"deferred_direction_lighting_vertex"];
            id <MTLFunction> directionalFragmentFunction = [deferredLightLibrary newFunctionWithName:@"deferred_directional_lighting_fragment"];
    ...
    {
        id <MTLLibrary> fairyLibrary = getLibrary(QString("://Shaders/libs/AAPLFairy.metallib"), _device);
        assert(fairyLibrary);

        id <MTLFunction> fairyVertexFunction = [fairyLibrary newFunctionWithName:@"fairy_vertex"];
        id <MTLFunction> fairyFragmentFunction = [fairyLibrary newFunctionWithName:@"fairy_fragment"];
    ...
    // Setup pipeline to draw sky
    {
        ...
        id <MTLLibrary> skyboxLibrary = getLibrary(QString("://Shaders/libs/AAPLSkybox.metallib"), _device);
        assert(skyboxLibrary);

        id <MTLFunction> skyboxVertexFunction = [skyboxLibrary newFunctionWithName:@"skybox_vertex"];
        id <MTLFunction> skyboxFragmentFunction = [skyboxLibrary newFunctionWithName:@"skybox_fragment"];
    ...
    id <MTLLibrary> shadowLibrary = getLibrary(QString("://Shaders/libs/AAPLShadow.metallib"), _device);
    assert(shadowLibrary);
    // Setup objects for shadow pass
    {
        // Create render state pipeline for shadow pass
        {
            id <MTLFunction> shadowVertexFunction = [shadowLibrary newFunctionWithName:@"shadow_vertex"];
    ...
    // Setup objects for point light mask rendering
    {
        ...
        {
            //id <MTLLibrary> defaultLibrary = [_device newDefaultLibrary]; XXXXXXXXXXXXXX Remove me
            id <MTLLibrary> pointLightLibrary = getLibrary(QString("://Shaders/libs/AAPLPointLights.metallib"), _device);
            assert(pointLightLibrary);

            id <MTLFunction> lightMaskVertex = [pointLightLibrary newFunctionWithName:@"light_mask_vertex"];
...
- (void)loadAssets
{
    ...
    NSURL *modelFileURL = nil;
    QTemporaryDir tempDir;
    if (tempDir.isValid())
    {
        const QString tempFile = tempDir.path() + "/Temple.obj";
        if (QFile::copy("://Images/Temple/Temple.obj", tempFile)) {
          modelFileURL = [NSURL URLWithString:[tempFile.toNSString() stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
        }
    }
    ...
    // Create a simple 2D triangle strip circle mesh for fairies
    {
        ...
        for(int vtx = 0; vtx < AAPLNumFairyVertices; vtx++)
        {
            ...
            vector_float2 position = {static_cast<float>(sin(point*angle)), static_cast<float>(cos(point*angle))};
    ...
    // Load textures for non mesh assets
    {
        ...
        QString skyBoxPath[6] =
        {
            QString("://Images/Skybox/X+.png"),
            QString("://Images/Skybox/X-.png"),
            QString("://Images/Skybox/Y+.png"),
            QString("://Images/Skybox/Y-.png"),
            QString("://Images/Skybox/Z+.png"),
            QString("://Images/Skybox/Z-.png"),
        };
        ...
        int texWidth, texHeight, texChannels;
        {
            ...
            if (tempDir.isValid()) {
              ...
              if (QFile::copy(skyBoxPath[0], tempFile)) {
                  //pPixels = stbi_load(tempFile.toStdString().c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
                  pPixels = stbi_load(tempFile.toStdString().c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
              }
            }
        }

        const CGFloat cubeSize = texWidth;
        ...
        _skyMap = [_device newTextureWithDescriptor:textureDescriptor];
        ...
        for (size_t slice = 0; slice < 6; ++slice)
        {
            ...
            if (tempDir.isValid()) {
              const QString tempFile = tempDir.path() + "/temp.png";
              if (QFile::copy(skyBoxPath[slice], tempFile)) {
                  pPixels = stbi_load(tempFile.toStdString().c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
              }
            }

            [_skyMap replaceRegion:region
                       mipmapLevel:0
                             slice:slice
                         withBytes:pPixels
...
//                                              error:&error];
        ...
        ////////////////////////////////////////////////////////////////////////////
#if LOAD_FAIRY_RESOURCE_WITH_QIMAGE == 1
        {
            QImage fairyImage("://Images/Fairy/fairy.png");
            MTLTextureDescriptor* textureDescriptor = [[MTLTextureDescriptor alloc] init];
            textureDescriptor.textureType = MTLTextureType2D;
            textureDescriptor.pixelFormat = MTLPixelFormatRGBA8Unorm;
            textureDescriptor.width = fairyImage.size().width();
            textureDescriptor.height = fairyImage.size().height();

            const uint8_t* imageBits = fairyImage.bits();
            int texWidth, texHeight, texChannels;

            QTemporaryDir tempDir;
            stbi_uc* pPixels = NULL;
            if (tempDir.isValid()) {
                const QString tempFile = tempDir.path() + "/fairy.png";
                if (QFile::copy("://Images/Fairy/fairy.png", tempFile))
                {
                    pPixels = stbi_load(tempFile.toStdString().c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
                }
            }

            _fairyMap = [_device newTextureWithDescriptor:textureDescriptor];
            NSUInteger bytesPerRow = 4 * fairyImage.width();

            MTLRegion region = { { 0, 0, 0 }, {(NSUInteger)fairyImage.width(), (NSUInteger)fairyImage.height(), 1} };
            [_fairyMap replaceRegion:region
                        mipmapLevel:0
                          withBytes:pPixels
                        bytesPerRow:bytesPerRow];
            free(pPixels);

//            NSData* fairydata = [[NSData alloc] initWithBytes:fairyImage.bits()
//                                                       length:/*4 * fairyImage.width() * fairyImage.height()*/fairyImage.byteCount() ];

//            _fairyMap = [textureLoader newTextureWithData:fairydata
//                                         options:textureLoaderOptions
//                                           error:&error];
//            free(fairydata);
...
//            if(!_fairyMap || error)
//            {
//                //if (fairyImage.format() == QImage::Format_Indexed8)
//                qDebug() << "->>>>>Fairy image Format: " << fairyImage.format() << "byteCount: " << fairyImage.bytesPerLine() * fairyImage.height()<< "dataSize: " << 4 * fairyImage.width() * fairyImage.height();
...
//            }
...
#else
        {
            ...
            if (tempDir.isValid())
            {
                ...
                if (QFile::copy("://Images/Fairy/fairy.png", tempFile))
                {
                    _fairyMap = [textureLoader newTextureWithContentsOfURL: [NSURL fileURLWithPath: tempFile.toNSString()]
                                                          options:textureLoaderOptions
                                                            error:&error];
                }
            }
...
- (void)updateLights:(matrix_float4x4)modelViewMatrix
{
    ...
    for(int i = 0; i < AAPLNumLights; i++)
    {
        ...
        if(i < AAPLTreeLights)
        {
            double lightPeriod = lightData[i].light_speed  * _frameNumber * 0.01f;
