...
//    id <MTLLibrary> defaultLibrary = [self.device newDefaultLibrary];
    ...
    {
        id <MTLLibrary> pointLightLibrary = getLibrary(QString("://Shaders/libs/AAPLPointLights.metallib"), self.device);
        assert(pointLightLibrary);

        id <MTLFunction> lightVertexFunction = [pointLightLibrary newFunctionWithName:@"deferred_point_lighting_vertex"];
        id <MTLFunction> lightFragmentFunction = [pointLightLibrary newFunctionWithName:@"deferred_point_lighting_fragment"];
    ...
    _GBufferRenderPassDescriptor.colorAttachments[AAPLRenderTargetLighting].storeAction = MTLStoreActionStore/*MTLLoadActionDontCare*/; // XXXXXX
#else
...
{
    if (size.width == 0 || size.height ==0) return; // Added for Qt
...
- (void) drawInMTKView:(nonnull MTKView *)view
{
    ...
    {
        static int mode = 0;
        [self toggleBufferExaminationMode: static_cast<AAPLExaminationMode>(/*++*/mode%6)];
        time = clock();
    }
